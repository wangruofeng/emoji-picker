...
- 🔍 **多语言 / 关键词搜索**：输入“笑”“heart”“火”等即时过滤 1000+ Emoji，搜索随当前语言适配。
...
- 📊 **码点详情**：点击 Emoji 打开详情面板，可放大为完整 Modal，展示 Unicode（十六/十进制）、UTF-8 字节、HTML 实体、CSS content、JS 转义等编码信息，以及分类、组合类型、码点拆解等属性；每项可单独复制，并支持复制 Emoji 与分享链接。
- 🏻 **肤色变体**：对支持肤色的手势类 Emoji，详情面板提供 6 种肤色快速切换复制。
- 🌓 **深浅色主题**：一键切换，偏好保存在 `localStorage emoji-theme`。
- 📱 **响应式布局**：桌面端详情面板在右侧，移动端在底部。
- 📲 **PWA 支持**：通过 HTTPS 或 localhost 访问时可安装为独立应用，并缓存应用资源以支持离线使用。
